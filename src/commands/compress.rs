...
impl CompressCommand {
    pub fn execute(file_path: &Path, overwrite: bool) -> Result<()> {
        ...
        println!("  • Tokens Saved:      {} ({:.1}% context reduction)", Formatters::format_tokens(report.tokens_saved).bold().green(), report.savings_percentage);
        println!("  • Output File:       {}", saved_path.display().bold().cyan());

...
const IGNORE_ENTRIES: &[(&str, &[&str])] = &[
    (
        ...
        &[
            ".env", ".env.*", "!.env.example", "*.pem", "*.key", "id_rsa", "id_ed25519",
    ...
    (
        ...
        &["node_modules/", ".next/", "dist/", "out/", ".turbo/", ".cache/"],
    ),
    (
        "Python caches",
...
impl FixerEngine {
    ...
    fn apply_ignore_file(path: &Path, dry_run: bool) -> Result<FixAction> {
        ...
        let added = block.lines().filter(|l| !l.trim().is_empty() && !l.starts_with('#')).count();
        ...
        Ok(FixAction {
            target: path.to_path_buf(),
            outcome: if exists { FixOutcome::Updated } else { FixOutcome::Created },
...
"#,
    ...
    pub fn compile_zshrc_bytecode(dry_run: bool) -> Result<FixAction> {
        ...
        let status = Command::new("zsh")
            .args(["-fc", "zcompile -R -- \"$HOME/.zshrc.zwc\" \"$HOME/.zshrc\""])
            .status()?;
        ...
        Ok(FixAction {
            target: zwc,
            outcome: if before == after { FixOutcome::AlreadyApplied } else { FixOutcome::Created },
...
mod tests {
    ...
    fn tempdir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("agentprof_fixer_{}_{}", tag, std::process::id()));
    ...
    fn test_existing_ignore_file_content_is_preserved() {
        ...
        assert!(after.contains("# my curated rules"), "user comment was dropped");
        assert!(after.contains("secret-project/"), "user pattern was dropped");
        assert!(after.contains("target/"), "agentprof patterns were not added");
        let _ = fs::remove_dir_all(&dir);
    }
    ...
    fn test_strip_managed_block_removes_only_the_block() {
        ...
        let stripped = FixerEngine::strip_managed_block(&content, IGNORE_BLOCK_START, IGNORE_BLOCK_END);
